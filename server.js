...
//     extended: true
// }))   //For Barsing url string and object and arrays
...
app.post('/purchase-order' , upload.single("PO_File") , async (req , res) => {
    ...
    // 4d56sa456a456
    try{
        let result_check = await sql.query(query_check)
        
        if(result_check.recordset.length > 0){
            res.json({ msg:'Purchase Order Exists'})
        } else {
            let query_insert = await sql.query(query)
            res.json({ msg:'Purchase Order Submitted'})
        }
    }
    catch(err){
        console.error(err)
...
app.get('/quote-count', (req, res) => {
    ...
    //Using promises for sql query instead of callbacks
    let query_result = sql.query(query).then(result => {
...
app.get('/online-count', async (req, res) => {
    ...
    let query_result = await sql.query(query)
    let values = Object.values(query_result.recordset[0])
...
app.get('/local-count', async (req, res) => {

    // let query = `SELECT COUNT(*) FROM Orders WHERE TypeOfOrder = 'local'`

    let query_result = await sql.query(`SELECT COUNT(*) FROM Orders WHERE TypeOfOrder = 'local'`)
    let values = Object.values(query_result.recordset[0])
    res.json(values[0])
    // let query_res = sql.query(query, (err, result) => {
    //     if (err) {
